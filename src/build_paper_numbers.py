...
SPEC = [
    ...
    # --- E101: alternative-curve exponent null, chord baseline, 1PN Fisher origin of the offset ---
    ("AltErrOaHalf",  E101, "D1_exponent_null.p_star.O4a.err_at_0.50", "f1"),
    ("AltErrObHalf",  E101, "D1_exponent_null.p_star.O4b.err_at_0.50", "f1"),
    ("AltRatioMin",   E101, "@altratio_min", "f1"),
    ("AltRatioMax",   E101, "@altratio_max", "f1"),
    ("AltPStarLo",    E101, "@altpstar_min", "f2"),
    ("AltPStarHi",    E101, "@altpstar_max", "f2"),
    ("ChordOa",       E101, "D2_chord_baseline.O4a.chord", "f2"),
    ("ChordOb",       E101, "D2_chord_baseline.O4b.chord", "f2"),
    ("OnePNzero",     E101, "D3_1PN_fisher.median_0PN_err_deg", "f1"),
    ("OnePNone",      E101, "D3_1PN_fisher.median_1PN_err_deg", "f1"),
    ("OnePNpeff",     E101, "D3_1PN_fisher.median_p_eff", "f3"),
    ("OnePNpeffLo",   E101, "@peff_lo", "f3"),
...
def derived(tag, d):
    ...
    if tag == "altratio_min" or tag == "altratio_max":
        r=[d["D1_exponent_null"]["p_star"][c]["err_at_0.50"]/d["D1_exponent_null"]["p_star"][c]["err_at_0.60"] for c in ("GWTC-3","O4a","O4b")]
        return min(r) if tag=="altratio_min" else max(r)
    if tag == "altpstar_min" or tag == "altpstar_max":
        r=[d["D1_exponent_null"]["p_star"][c]["p_star"] for c in ("GWTC-3","O4a","O4b")]
        return min(r) if tag=="altpstar_min" else max(r)
    if tag == "peff_lo":
        return d["D3_1PN_fisher"]["p_eff_iqr"][0]
    if tag == "peff_hi":
        return d["D3_1PN_fisher"]["p_eff_iqr"][1]
    if tag == "naivesigma":
...
def main():
    ...
    for macro, art, path, how in SPEC:
        ...
        _src = art if art.endswith(".json") else f"results/{art}_results.json"
        prov.append(f"% {macro:<22} {_src} :: {path}")
