...
There is a sharper way to say what the tangent term is. A Gaussian posterior's principal axis \emph{is}
the local tangent, so the tangent error measures exactly the error made by the asymptotic Gaussian
(Bernstein--von Mises) limit in which the posterior tends to a normal distribution with the inverse
Fisher covariance. On elongated events that error is $5.02^\circ$, against $1.03^\circ$ once the arc is
